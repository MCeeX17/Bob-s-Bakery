...
(function () {
    ...
    function productWaLink(product) {
        return waLink(`Hey! I'd like to enquire about the *${product.name}* (${product.price}). Is it available? 😊`);
    }
    ...
    function openModal(productId) {
        ...
        // Pre-order notice — inject after tags if not already present
        let notice = document.getElementById('modal-preorder-notice');
        if (!notice) {
            notice = document.createElement('p');
            notice.id = 'modal-preorder-notice';
            notice.className = 'modal-preorder-notice';
            notice.innerHTML = '⚠️ Pre-day order required — please order the day before you need it.';
            modalTags.parentNode.insertBefore(notice, modalTags.nextSibling);
        }
